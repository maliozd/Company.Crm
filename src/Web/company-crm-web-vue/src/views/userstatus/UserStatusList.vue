...
function fetchItems(page = 1) {
	axios.get('userstatus/getpaged', { params: { page, perPage: 10 } }).then(response => {
...
function editItem(id) {
	axios.get('userstatus/' + id).then(response => {
		...
		<div class="container-xl">
			<div class="card">
				<div class="card-header">
					<h3 class="card-title">UserStatus</h3>
			...
			<UserStatusModal :item="dataItem" @onSaved="itemSaved"></UserStatusModal>
		</div>
	</div>
</template>